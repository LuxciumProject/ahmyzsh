...
if [[ -L "$target_apps_dir$desktop_file_name" ]]; then
  ...
  if [[ "$answer" == "y" ]]; then
    ...
    rm "$target_apps_dir/$desktop_file_name"
  else
    exit 0
  fi
elif [[ -e "$target_apps_dir$desktop_file_name" ]]; then
  echo "Error: A file or directory with the name '$desktop_file_name' already exists in the user-level applications folder."
  exit 1
fi

# Create the symlink in the user-level applications folder
ln -s "$desktop_file_path" "$target_apps_dir/$desktop_file_name"
