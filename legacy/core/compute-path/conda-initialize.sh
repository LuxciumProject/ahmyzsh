...
function conda_init_minerva3() {
  # >>> conda initialize >>>
  # !! Contents within this block are managed by 'conda init' !!
  __conda_setup="$('/home/luxcium/minerva3/bin/conda' 'shell.bash' 'hook' 2>/dev/null)"
  if [ $? -eq 0 ]; then
    eval "$__conda_setup"
  else
    if [ -f "/home/luxcium/minerva3/etc/profile.d/conda.sh" ]; then
      . "/home/luxcium/minerva3/etc/profile.d/conda.sh"
    else
      export PATH="/home/luxcium/minerva3/bin:$PATH"
    fi
  fi
  unset __conda_setup
  # <<< conda initialize <<<
}
function conda_init_() {
  # # >>> conda initialize >>>
  # # !! Contents within this block are managed by 'conda init' !!
